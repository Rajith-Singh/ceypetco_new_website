...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => lebuild_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => lebuild_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => lebuild_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => lebuild_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => lebuild_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => lebuild_meta( 'banner_elementor_template', $page_id ),
				'banner'            => lebuild_set( lebuild_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => lebuild_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => lebuild_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : lebuild_the_title( $template ),
				'banner'        => lebuild_set( $bg, 'url' ),
