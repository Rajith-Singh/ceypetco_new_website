...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'lebuild' . '_options-mods' );
		...
		$scripts = apply_filters( 'LEBUILD/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( LEBUILD_NONCE ),
		);

		wp_localize_script( 'jquery', 'lebuild_data', $header_data );

		if ( lebuild_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', lebuild_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'LEBUILD/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "lebuild-{$name}", $style );
			} else {
				wp_enqueue_style( "lebuild-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = lebuild_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= lebuild_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'lebuild-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'lebuild-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$settings = get_theme_mod( 'lebuild' . '_options-mods' );

		if ( $custom_font = lebuild_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('lebuild_redux_custom_fonts_load', $custom_font );
