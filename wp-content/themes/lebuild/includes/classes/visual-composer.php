...
class Visual_Composer {
	...
	function init() {
		...
		$maps = apply_filters( 'lebuild_vc_map', $this->maps );
		...
		foreach ( $maps as $value) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( function_exists( 'wplebuild_shortcode' ) ) {
					$tag = esc_attr( lebuild_set( $data, 'base' ) );

					wplebuild_shortcode( $tag, array( $this, 'output' ) );
				}
			}
		}
	}
	...
	function output( $atts, $content = null, $tag ) {
		...
		lebuild_template_load( 'shortcodes/' . $tag . '.php', compact( 'atts', 'content', 'tag' ) );
	...
	function get_file( $tag ) {
		...
		$file = lebuild_template( 'includes/resource/vc_map/'.$tag . '.php' );
		$file = apply_filters( "lebuild_vc_map_file_{$tag}", $file );
	...
	function shortcodeParams( $tag ) {
		...
		if ( file_exists( $file ) ) {
			...
			return lebuild_set( $data, 'params' );
		}
	...
	function toggle( $settings, $value ) {
		...
		$values = isset( $settings['value'] ) && is_array( $settings['value'] ) ? $settings['value'] : array( esc_html__( 'Yes', 'lebuild' ) => 'true' );
		if ( ! empty( $values ) ) {
